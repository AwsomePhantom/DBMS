...
$incidents_list = CONNECTION->getIncidentReports($user_obj->id);
?>
...
ENDL_;
                    ...
                    foreach($incidents_list as $incident) {
                        ...
                        if($user_obj->id === $incident['user_id'])  {
                            ...
                            echo "<tr><td colspan='3'><button name='deleteIncidentButton' class='btn btn-danger' type='submit' value=''><i class='bi bi-x-square-fill'> Delete request</button></td></tr>";
                        }
                        echo "<tr><td colspan='3'><strong>Repair offers</strong></td></tr>";
                        if(!empty($repair_requests)) {
                            foreach ($repair_requests as $request) {
                                echo "<tr><td><strong>Company:</strong> {$request['company_name']}</td><td>Company Type:</strong> {$request['company_type']}</td><td><strong>Email:</strong> {$request['email']}</td></tr>";
                                echo "<tr><td><strong>Address:</strong> {$request['address']}</td><td>City:</strong> {$request['city']}</td><td><strong>Country:</strong> {$request['country_code']}</td></tr>";
                                echo "<tr><td colspan='3'><strong>Options</strong></td></tr>";
                                echo "<tr><td colspan='3'><button name='company_name' class='btn btn-success' type='submit' value='{$request['company_name']}'><i class='fa-solid fa-thumbs-up'></i> Accept Offer</button></td></tr>";
